...
type SvcClient interface {
	dummy1(ctx context.Context, params SvcRequest) error
	dummy2(ctx context.Context, params SvcRequest) error
	dummy5(ctx context.Context, foo int, params SvcRequest) error
}
...
func (c *svcClient) dummy1(ctx context.Context, params SvcRequest) error {
...
func (c *svcClient) dummy2(ctx context.Context, params SvcRequest) error {
	// Convert our params into the objects we need for the request
	reqEncoder := &serde{}
	...
	_, err := callAPI(ctx, c.base, "POST", fmt.Sprintf("/other_dummy?%s", queryString.Encode()), headers, body, nil)
	return err
}

func (c *svcClient) dummy5(ctx context.Context, foo int, params SvcRequest) error {
	// Convert our params into the objects we need for the request
	reqEncoder := &serde{}
	...
	_, err := callAPI(ctx, c.base, "POST", fmt.Sprintf("/dummy/%d?%s", foo, queryString.Encode()), headers, body, nil)
	return err
}
